...
# ══════════════════════════════════════════════════════════════════════════════
#  GLOBAL CSS - DARK LIQUID GLASS EDITION (Enhanced Colors)
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
:root {
    ...
    --cyan-bdr:     rgba(0,242,254,0.35);
    
    --violet:       #9d4edd;
    --violet-dim:   rgba(157,78,221,0.2);
    
    --burgundy:     #ff0844;
    --burgundy-dim: rgba(255,8,68,0.15);
...
.stApp {
    ...
    background-image: 
        radial-gradient(circle at 10% 20%, var(--violet-dim), transparent 40%),
        radial-gradient(circle at 90% 80%, var(--blue-dim), transparent 45%),
        radial-gradient(circle at 50% 50%, var(--cyan-dim), transparent 60%);
...
.hero-icon { font-size: 3rem; line-height: 1; margin-bottom: 0.7rem; filter: drop-shadow(0 0 15px rgba(41,121,255,0.6)); }
.hero-title { font-size: clamp(1.4rem, 3.5vw, 2.2rem); font-weight: 900; color: var(--text); line-height: 1.45; margin-bottom: 0.4rem; }
.hero-title span { background: linear-gradient(to left, var(--cyan), var(--blue)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-shadow: 0 0 20px rgba(41,121,255,0.4); }
.hero-sub  { color: var(--text-2); font-size: 0.95rem; font-weight: 500; letter-spacing: 0.03em; }
.hero-pill { display: inline-block; margin-top: 1.1rem; background: rgba(41,121,255,0.15); border: 1px solid var(--blue-bdr); border-radius: 50px; padding: 0.35rem 1.2rem; font-size: 0.75rem; font-weight: 800; color: var(--cyan); letter-spacing: 0.10em; backdrop-filter: blur(10px); }

div[data-testid="stBaseButton-primary"] button, button[kind="primary"] {
    width: 100% !important; background: linear-gradient(135deg, rgba(41,121,255,0.25) 0%, rgba(41,121,255,0.05) 100%) !important; backdrop-filter: blur(12px) !important; border: 1px solid var(--blue-bdr) !important; border-top: 1px solid rgba(255,255,255,0.3) !important; color: #fff !important; font-family: 'Noto Sans Arabic', sans-serif !important; font-size: 1.05rem !important; font-weight: 900 !important; border-radius: 16px !important; padding: 0.85rem 1.2rem !important; letter-spacing: 0.05em !important; box-shadow: 0 4px 20px rgba(0,0,0,0.5) !important; transition: all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1) !important;
}
div[data-testid="stBaseButton-primary"] button:hover, button[kind="primary"]:hover { transform: translateY(-3px) !important; background: linear-gradient(135deg, rgba(41,121,255,0.4) 0%, rgba(41,121,255,0.1) 100%) !important; box-shadow: 0 8px 30px rgba(41,121,255,0.35) !important; text-shadow: 0 0 10px rgba(255,255,255,0.5) !important; }
...
div[data-testid="stBaseButton-secondary"] button:hover, button[kind="secondary"]:hover { transform: translateY(-2px) !important; background: rgba(0,242,254,0.15) !important; border-color: var(--cyan-bdr) !important; color: #fff !important; }

.sec-head { display: flex; align-items: center; gap: 0.8rem; margin-bottom: 1.2rem; }
.sec-head-line { flex: 1; height: 1px; background: linear-gradient(90deg, rgba(255,255,255,0.3) 0%, transparent 100%); }
.sec-head-text { color: #fff; font-size: 0.85rem; font-weight: 800; letter-spacing: 0.10em; white-space: nowrap; text-shadow: 0 0 10px rgba(255,255,255,0.4); }
...
button[data-baseweb="tab"][aria-selected="true"] p { color: var(--cyan) !important; text-shadow: 0 0 10px rgba(0,242,254,0.5); }
div[data-baseweb="tab-highlight"] { background-color: var(--cyan) !important; box-shadow: 0 0 10px rgba(0,242,254,0.6) !important; height: 3px !important; border-radius: 3px 3px 0 0 !important; }
...
.stNumberInput input:focus, .stSelectbox > div > div:focus { 
    ...
    box-shadow: 0 0 0 2px rgba(0,242,254,0.3), inset 0 2px 5px rgba(0,0,0,0.6) !important; 
    outline: none !important; 
    background-color: rgba(0,0,0,0.7) !important;
}

div[data-testid="stSlider"] { direction: ltr !important; padding: 0 0.2rem; }
div[data-testid="stSlider"] .rc-slider-rail, .stSlider .rc-slider-rail { background: rgba(0,0,0,0.6) !important; border-radius: 6px !important; height: 8px !important; box-shadow: inset 0 1px 3px rgba(0,0,0,0.8); }
div[data-testid="stSlider"] .rc-slider-track, .stSlider .rc-slider-track { background: linear-gradient(90deg, var(--violet), var(--cyan)) !important; height: 8px !important; border-radius: 6px !important; }
div[data-testid="stSlider"] .rc-slider-handle, .stSlider .rc-slider-handle { width: 22px !important; height: 22px !important; margin-top: -7px !important; background: #fff !important; border: 4px solid var(--cyan) !important; box-shadow: 0 0 15px rgba(0,242,254,0.6) !important; }

/* Small Helper Text */
.helper-text { font-size: 0.75rem; color: rgba(255,255,255,0.5); margin-top: -10px; margin-bottom: 15px; direction: rtl; text-align: right;}

.summary-card { background: rgba(255,255,255,0.04); backdrop-filter: blur(10px); border: 1px solid var(--glass-border); border-radius: 14px; padding: 1.2rem; margin-top: 1rem; direction: rtl; }
.summary-card-title { color: var(--cyan); font-size: 0.8rem; font-weight: 800; margin-bottom: 0.8rem; letter-spacing: 0.05em; }
...
.rc-low   { border-top: 2px solid var(--green); background: linear-gradient(180deg, rgba(0,230,118,0.15) 0%, var(--glass-bg) 100%); }
.rc-high  { border-top: 2px solid var(--red); background: linear-gradient(180deg, rgba(255,23,68,0.15) 0%, var(--glass-bg) 100%); }
.rc-limit { border-top: 2px solid var(--cyan); background: linear-gradient(180deg, rgba(0,242,254,0.15) 0%, var(--glass-bg) 100%); }
.rc-eyebrow { font-size: 0.8rem; font-weight: 800; letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 1rem; }
.rc-value   { font-size: clamp(2.2rem, 5vw, 3.5rem); font-weight: 900; line-height: 1.1; margin-bottom: 0.3rem; letter-spacing: -0.02em; text-shadow: 0 4px 15px rgba(0,0,0,0.5); }
.rc-limit .rc-value { background: linear-gradient(to right, #fff, var(--cyan)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
...
.badge-low   { background: rgba(0,230,118,0.15); color: var(--green); border: 1px solid var(--green-dim); }
.badge-high  { background: rgba(255,23,68,0.15); color: var(--red); border: 1px solid var(--red-dim); }
.badge-limit { background: rgba(0,242,254,0.15); color: var(--cyan); border: 1px solid var(--cyan-dim); }

.metric-card { padding: 1rem 0.5rem; text-align: center; transition: transform 0.3s ease; border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; background: rgba(0,0,0,0.4); backdrop-filter: blur(10px);}
.metric-card:hover { transform: translateY(-3px); border-color: rgba(0,242,254,0.4); box-shadow: 0 5px 15px rgba(0,242,254,0.1); }
...
.about-card-title { color: var(--cyan); font-size: 0.9rem; font-weight: 900; letter-spacing: 0.05em; margin-bottom: 0.8rem; padding-bottom: 0.6rem; border-bottom: 1px solid rgba(255,255,255,0.1); }
.about-card-body { color: var(--text-2); font-size: 0.88rem; line-height: 1.9; }
.about-card-body b { color: #fff; font-weight: 800; }
.tech-tag { display: inline-block; background: rgba(255,255,255,0.08); color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 8px; padding: 0.3rem 0.8rem; font-size: 0.75rem; font-weight: 800; margin: 0.25rem 0.15rem; backdrop-filter: blur(5px); }
.about-center { text-align: center; padding: 1.5rem 0 1rem; }
.about-center-icon { font-size: 3rem; margin-bottom: 0.5rem; filter: drop-shadow(0 0 15px rgba(0,242,254,0.6)); }
.about-center-name { color: var(--cyan); font-size: 1.1rem; font-weight: 900; letter-spacing: 0.08em; text-transform: uppercase; font-family: 'Inter', sans-serif;}
...
div[role="listbox"] li:hover, div[role="listbox"] li[aria-selected="true"] { background-color: rgba(0,242,254,0.2) !important; }
...
.info-title { font-weight: 800; color: var(--cyan); margin-bottom: 0.6rem; font-size: 1rem; }
...
.footer-section strong { color: var(--cyan); font-weight: 700; }
...
def dataset_model_info_dialog():
    ...
    <div class="info-box">
        ...
        <div class="info-text">
            ئەم داتاسێتە سەرەتا لە <b>1200 کۆرپوس (ڕیز)</b> پێکدێت کە تایبەتە بە بازرگانی (B2B) کۆگاکانی هەولێر. لەبەر ئەوەی زۆربەی کڕیارەکان لە یەک جۆر بوون، بۆ دروستکردنی مۆدێلێکی بێلایەن، تەکنیکی <b>SMOTE</b> بەکارهێنراوە بۆ هاوسەنگکردنەوەی داتاکان، کە بەهۆیەوە قەبارەی داتاسێتەکە بەرزبووەتەوە بۆ <b>زیاتر لە 1500 کۆرپوس</b> بە یەکسانی تەواو لە نێوان هەردوو جۆری مەترسیدار و باوەڕپێکراودا.<br>
    ...
    <div class="info-box" style="border-left-color: #00f2fe; background: rgba(0, 242, 254, 0.1);">
        <div class="info-title" style="color: #00f2fe;">⚙️ مۆدێلەکانی ڕاهێنان (XGBoost)</div>
        <div class="info-text">
...
def model_evaluation_dialog():
    ...
    with tab_test:
        ...
        st.markdown("""
        <div style="background: rgba(0, 242, 254, 0.1); border-left: 4px solid var(--cyan); padding: 1rem; border-radius: 8px 0 0 8px; margin-top: 1.5rem;">
            <b>💡 ئەنجامی کۆتایی:</b> مۆدێلەکەمان بە سەرکەوتوویی توانای پێشبینیکردنی هەیە بە ڕێژەی زیاتر لە %87 بۆ مەترسی و %90 بۆ بڕی قەرز لەسەر داتای نەبینراو (Test). ئەنجامەکانی Test و Train زۆر نزیکن لە یەکەوە، ئەمەش دەریدەخات کە مۆدێلەکە زۆر جێگیرە و کێشەی (Overfitting)ی نییە.
        </div>
        """, unsafe_allow_html=True)
...
def project_info_dialog():
    ...
    <div class="info-box" style="border-left-color: #00f2fe; background: rgba(0, 242, 254, 0.1);">
        <div class="info-title" style="color: #00f2fe;">💡 چارەسەرەکە (The Solution)</div>
        <div class="info-text">
    ...
    with info_col1:
        ...
        <div class="about-card liquid-glass">
            ...
            <div class="about-card-body">
                ...
                🔗 <a href="https://github.com/UMEDJAMALA" target="_blank" style="color: #00f2fe; text-decoration: none;"><b>GitHub Profile (@UMEDJAMALA)</b></a><br>
                🔗 <a href="https://facebook.com/umedjamala" target="_blank" style="color: #00f2fe; text-decoration: none;"><b>Facebook Profile</b></a><br>
                📧 <a href="mailto:umedjamal2005@gmail.com" style="color: #00f2fe; text-decoration: none;"><b>Contact Email</b></a>
            </div>
        </div>
        """, unsafe_allow_html=True)
...
if not models_loaded:
    st.markdown("""
    <div class="liquid-glass" style="padding: 1rem; border-color: rgba(255,23,68,0.4); color: #ff1744; text-align: center; margin-bottom: 1.5rem; margin-top: 1.5rem;">
...
with col_l:
    ...
    avg_invoice = st.number_input("تێکڕای بەهای یەک وەسڵ ($)", min_value=0.0, value=250.0, step=10.0, format="%.0f")
    st.markdown('<div class="helper-text">زۆربەی وەسڵەکانی ئەم دوکانە چەند دۆلارە؟ (ئاسایی 50 بۆ 500)</div>', unsafe_allow_html=True)
    
    freq_per_month = st.number_input("تێکڕای وەسڵەکان لە مانگێکدا (دانە)", min_value=0.0, value=5.0, step=1.0)
    st.markdown('<div class="helper-text">لە مانگێکدا نزیکەی چەند جار کاڵا دەبات؟ (ئاسایی 2 بۆ 15)</div>', unsafe_allow_html=True)
    
    total_volume = st.number_input("کۆی قەبارەی بازرگانی ($)", min_value=0.0, value=5000.0, step=100.0, format="%.0f")
    st.markdown('<div class="helper-text">کۆی ئەو پارەیەی تا ئێستا کڕینی پێ کردووە چەندە؟</div>', unsafe_allow_html=True)
    
    unpaid_ratio_display = st.slider("ڕێژەی وەسڵە نەدراوەکان لەسەدا (%)", min_value=0, max_value=100, value=5, step=1)
...
if analyze:
    ...
    if models_loaded:
        ...
        credit_limit = max(0.0, avg_invoice * freq_per_month * 0.5)
